...
from htpp_data_coord import htpp_data_coord
from htpp_data_evol import htpp_data_evol
from htpp_data_nodal_mesh_files import htpp_data_nodal_mesh_files
    ...
    for file in files:
        ...
        for line in data_options:
            if '**STRAIN' in line[0].upper():
                ...
                all_data.append(line)
            elif '**COORD' in line[0].upper():
                all_data.append(line)
            elif '**EVOL' in line[0].upper():
                all_data.append(line)
            elif '**MESH' in line[0].upper():
                all_data.append(line)
        ...
        if len(all_data) > 0:
            ...
            #s#hutil.rmtree(path + '/htpp_output/' + name)
        ...
        if len(all_data) > 0:
            ...
            for option in all_data:
                ...
                if opt == '**STRAIN':
                    info = 1
                    htpp_data_strain(odb,ioi,option,section,name,info, 1)
                elif opt == '**STRESS':
                    info=1
                    htpp_data_stress(odb,ioi,option,section,name, info, 1)
                elif opt == '**ROTATION':
                    ...
                    htpp_data_coord(odb,ioi,option,section,name)
                elif opt == '**EVOL':
                    htpp_data_evol(odb,ioi,option,section,name)
                elif opt == '**MESH':
                    htpp_data_nodal_mesh_files(odb,ioi,option,section,name)
