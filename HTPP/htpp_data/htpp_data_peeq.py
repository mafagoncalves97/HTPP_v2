...
def htpp_data_peeq_option(odb,option):
	...
	yls='\n'.join(s for s in option if 'yls=' in s).replace(",", "").replace("\n", "")[5:-1].strip().split(';')
	yls = [int(s) for s in yls]
	return step,frame,elem_set, yls

# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------- PEEQ
# ------------------------------------------------------------------------------
def htpp_data_peeq(odb,ioi,option,section,odb_name):
	step,frame,elem_set,yls = htpp_data_peeq_option(odb,option)
...
# 						else:
# 							vals = odb_data.getSubset(region=soi,position=pos, sectionPoint=section[-1]).values
# 						strings = ['peeq',set.lower(),'s'+str(l),'f'+str(ls)]
# 						fname = dir+'_'.join(strings)+'.dat'
# 						with open(fname,'w') as f:
# 							for i in range(len(vals)):
# 								f.write('%f\n' %vals[i].data)
