...
from htpp_plot_indicator import htpp_plot_indicator
from htpp_plot_identifiability import htpp_plot_identifiability
from htpp_plot_colormap import htpp_plot_colormap
from htpp_plot_DIC_FEA_diff_colormap import htpp_plot_DIC_FEA_diff_colormap
from htpp_plot_BBindicator import htpp_plot_BBindicator
from htpp_plot_schmitt import htpp_plot_schmitt
	...
	for file in files:
		...
		for line in data_options:
			if '**STRAIN' in line[0].upper():
				...
				all_data.append(line)
			elif '**INDICATOR' in line[0].upper():
				all_data.append(line)
			elif '**IDENTIFIABILITY' in line[0].upper():
				all_data.append(line)
			elif '**COLORMAP' in line[0].upper():
				all_data.append(line)
			elif '**BBINDICATOR' in line[0].upper():
				all_data.append(line)
			elif '**DICFEACOLORMAP' in line[0].upper():
				all_data.append(line)
		...
		if len(all_data) > 0:
			...
			for option in all_data:
				...
				if opt == '**STRAIN':
					...
					pdf,table = htpp_plot_schmitt(name,option,pdf,size,table)
					...
					pdf,table = htpp_plot_indicator(name,option,pdf,size,table)
				elif opt == '**IDENTIFIABILITY':
					pdf,table = htpp_plot_identifiability(name,option,pdf,size,table)
				elif opt == '**COLORMAP':
					pdf,table = htpp_plot_colormap(name,option,pdf,size,table)
				elif opt == '**BBINDICATOR':
					pdf,table = htpp_plot_BBindicator(name,option,pdf,size,table)
				elif opt == '**DICFEACOLORMAP':
					pdf,table = htpp_plot_DIC_FEA_diff_colormap(name,option,pdf,size,table)
