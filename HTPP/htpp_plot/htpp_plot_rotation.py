...
def htpp_plot_rotation_aux2(bin_elas,bin_plas,peeq,obs,lims,n,odb_name):
	...
	plt.tight_layout()
	#plt.show()
	#plt.savefig(r'htpp_output/Figures/rotation_'+odb_name+'.jpg')
	#plt.close()
...
def htpp_plot_rotation_aux1(rotAngle,peeq,odb_name):
	fig = plt.figure()
	ax = fig.add_subplot(111)
	...
	std = statistics.stdev(rotAngle.tolist())  # STANDARD DEVIATION OF STRAIN STATE (dispersion of E2E1 ratio)
	u = statistics.mean(rotAngle.tolist())  # STANDARD DEVIATION OF STRAIN STATE (dispersion of E2E1 ratio)
	#print(std,u)
	



	# -------------------------- COMPUTE ROTATION ANGLE QUANTITATIVE INDICATOR (M.OLIVEIRA)
	bins_opt = 200
	rotAngle_norm = rotAngle/90
	h_norm = plt.hist(rotAngle_norm,bins_opt)
	count2 = h_norm[0]
	
	A = np.array([rotAngle_norm,peeq])
	B = A[:,np.argsort(A[0,:])]
	k = 0
	j = 0
	...
	for i in range(len(count2)):
		...
		for n in range(12):
			...
			for m in range(int(count2[i])):
				if temp[1,m] > binsPeeq2[n] and temp[1,m] <= binsPeeq2[n+1]:
					cont = cont + 1
				elif temp[1,m] == 0.0:
					contEls = contEls + 1
			freqBin_Plas2[i,n] = int(cont)
	...
	h_norm2 = np.sum(freqBin_Plas2,axis=1)
	h_norm2 = h_norm2/sum(h_norm2)
	print(h_norm2)
	dref = 0.005
	SR = 0
	for i in range(0,len(h_norm[0])):
		SR = SR + abs(dref - h_norm2[i])
	SR = 1- SR/2
	print(odb_name,SR*100)
...
def htpp_plot_rotation(odb_name,option,pdf,size,table):
	...
	size = [[[-0.1,-0.2],[6.0,4.6]],[[0.08,-0.1],[7.725,4.675]],[[0.08,-0.1],[5.725,4.675]]]
		...
		for l in step:
			for f in frame:
				for n in num:
					...
					bin_elas,bin_plas,obs = htpp_plot_rotation_aux1(rotAngle ,peeq, odb_name)

					fig = htpp_plot_rotation_aux2(bin_elas,bin_plas,peeq,obs, lims,n,odb_name)
