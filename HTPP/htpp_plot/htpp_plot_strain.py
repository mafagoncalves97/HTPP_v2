...
def htpp_plot_strain_aux(strain,peeq,rot,lims,txt,n,odb_name):
	...
	plt.text(-tmp*p[1],0.9*tmp*p[1],name,ha=al,va=al,fontsize=fnt,bbox=box,zorder=9)

	name = 'uniaxial\ntension'
	plt.plot(tens[0],tens[1],'k-',lw=0.5,zorder=0)
	plt.text(-0.16,0.37,name,ha=al,va=al,ma=al,fontsize=fnt,bbox=box)

	name = 'plane strain\ntension'
	plt.plot([0,0],[0,0.4],'k-',lw=0.5,zorder=0)
	plt.text(0,ylims[1]*p[3],name,ha=al,va=al,ma=al,fontsize=fnt,bbox=box)
 
	name = 'plane strain\ncompression'
	plt.plot([0,0],[-0.4,0],'k-',lw=0.5,zorder=0)
	plt.text(-0.32,0.02,name,ha=al,va=al,ma=al,fontsize=fnt,bbox=box)

	name = 'equibiaxial\ntension'
	plt.plot(eqBiax[0],eqBiax[1],'k-',lw=0.5,zorder=0)
	plt.text(0.3,0.35,name,ha=al,va=al,ma=al,fontsize=fnt,bbox=box)
	...
	if n == 1:
		...
		pl = plt.scatter(s_plas[:,1],s_plas[:,0],s=8.5,c=p_plas,cmap=cjet, zorder=100,rasterized=rast,linewidths=0.5, norm=plt.Normalize(vmin=0,vmax=0.26))
		...
		el = plt.scatter(s_elas[:,1],s_elas[:,0],s=1.5,color=grey, edgecolors='none',linewidths=0.2,zorder=200,rasterized=rast)
		...
		cjet = mpl.cm.get_cmap('jet', 12)
		pl = plt.scatter(s_plas[:,1],s_plas[:,0],s=3.5,c=rot_plas,cmap=cjet, edgecolors=black_02,linewidths=0.1,zorder=200,rasterized=rast, norm=mpl.colors.Normalize(vmin=0.0,vmax=90.0))
	...
	plt.xticks(np.arange(xlims[0],xlims[1]+0.05,0.1))
	plt.tight_layout()
	plt.show()
	#plt.gcf().set_size_inches(7,5)
	plt.savefig(r'htpp_output/Figures/strain_diagram_bar_'+odb_name+'.jpg',bbox_inches='tight')
	#clb.remove()
	#plt.savefig(r'htpp_output/Figures/strain_diagram_'+odb_name+'.jpg',bbox_inches='tight')
	plt.close()
